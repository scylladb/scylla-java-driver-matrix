...
class Run:
    ...
    def _run(self):
        ...
        logging.info(cmd)
        subprocess.call(cmd, env=self._environment(), cwd=self._java_driver_git)
        ...
        if self._tag.startswith('3.7'):
            ...
            subprocess.call(cmd, env=self._environment(), cwd=self._java_driver_git)
            if  self._scylla_version:
                cmd = ["bash",  "-c",  "mvn -B -pl driver-core -Dtest.groups='long' -Dtest='{0}*' test -Dscylla.version={1}".format(exclude_str, self._scylla_version)]
            elif self._scylla_install_dir:
                cmd = ["bash",  "-c",  "mvn -B -pl driver-core -Dtest.groups='long' -Dtest='{0}*' test -Dccm.directory={1}".format(exclude_str, self._scylla_install_dir)]
            else:
            ...
            if  self._scylla_version:
                cmd = ["bash",  "-c",  "mvn -B -pl integration-tests -Dtest='{0}*' test -Dscylla.version={1}".format(exclude_str, self._scylla_version)]
            elif self._scylla_install_dir:
                cmd = ["bash",  "-c",  "mvn -B -pl integration-tests -Dtest='{0}*' test -Dccm.directory={1}".format(exclude_str, self._scylla_install_dir)]
            else:
                raise ValueError("No scylla version or cassaandra dir defined")
    ...
    def _process_output(self):
        ...
        return subprocess.run(["bash", "-c", "grep -nr 'ERROR!\|FAILURE!'  ../{0}/*.txt".format(self._tag)],
                universal_newlines=True, cwd=self._java_driver_git, stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout